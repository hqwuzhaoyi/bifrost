...
type OAuth2ServerConfig struct {
	...
	// existing vk-mode grants stop refreshing (they expire within one
	// AccessTokenTTL). Anonymous session identity is unaffected — that is
	// governed separately by EnforceAuthOnInference. Honored only when an
	// identity provider is configured, so it can never strip the consent flow of
	// every identity option. Only meaningful when MCPServerAuthMode is oauth.
