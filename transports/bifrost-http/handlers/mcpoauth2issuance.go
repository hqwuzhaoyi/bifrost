...
func (h *OAuth2IssuanceHandler) handleTokenRefresh(ctx *fasthttp.RequestCtx) {
	...
	// VK identity cutoff: when virtual-key identity has been disabled, vk-mode
	// grants must stop refreshing so existing integrations fall out within one
	// access-token TTL. Gated on user identity being available, identical to the
	// consent flow's availableModes — so this can never fire where vk is still
	// the offered path.
